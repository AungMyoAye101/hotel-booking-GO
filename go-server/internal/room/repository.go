...
func (r *Repository) FindByID(id uuid.UUID) (*models.Room, error) {
	...
	if err := r.db.Preload("Photo").First(&room, "id = ?", id).Error; err != nil {
...
func (r *Repository) FindAll(offset, limit int) ([]models.Room, int64, error) {
	...
	if err := r.db.Preload("Photo").Order("created_at desc").Offset(offset).Limit(limit).Find(&rooms).Error; err != nil {
		return nil, 0, err
	}
