...
type Payment struct {
	...
	Status        string         `gorm:"type:varchar(50);not null;default:'PENDING';index:idx_status_paid" json:"status" validate:"required,oneof=PENDING PAID FAILED REFUNDED"`
	Amount        float64        `gorm:"type:numeric(12,2);not null" json:"amount" validate:"required,gt=0"`
	PaidAt        time.Time      `gorm:"autoCreateTime;index:idx_status_paid" json:"paid_at"`
